...
export default function Suggest() {
  ...
  if (!isApproved) return <div className="p-8 text-center text-sm text-muted-foreground">Tài khoản cần được admin duyệt trước</div>;

  const submit = async (e: React.FormEvent) => {
    e.preventDefault(); setL(true);
    const { error } = await supabase.from("suggestions").insert({
      user_id: user.id, business_name: name, business_type: type, contact_info: contact, description: desc,
    });
    setL(false);
      ...
      <div className="flex flex-wrap gap-2">
        {BUSINESS_TYPES.map(t => (
          <button type="button" key={t} onClick={() => setType(t)}
            className={cn("px-3 py-1.5 rounded-full text-sm border", type === t ? "bg-primary text-primary-foreground border-primary" : "bg-card")}>
