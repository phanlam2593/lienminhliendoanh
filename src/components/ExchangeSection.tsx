...
const STATUS_COLOR: Record<ExchangeStatus, { dot: string; chip: string; label: string }> = {
  pending:        { dot: "bg-amber-400",   chip: "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200",   label: "Chờ phản hồi" },
  accepted:       { dot: "bg-sky-400",     chip: "bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-200",           label: "Đã chấp nhận" },
  requester_done: { dot: "bg-sky-500",     chip: "bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-200",           label: "Người gửi xong" },
  receiver_done:  { dot: "bg-sky-500",     chip: "bg-sky-100 text-sky-800 dark:bg-sky-900/40 dark:text-sky-200",           label: "Người nhận xong" },
  completed:      { dot: "bg-emerald-500", chip: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200", label: "Hoàn thành" },
  rejected:       { dot: "bg-rose-500",    chip: "bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-200",       label: "Đã từ chối" },
...
const STATUS_STEP: Record<ExchangeStatus, number> = {
  pending: 1, accepted: 2, requester_done: 3, receiver_done: 4, completed: 6, rejected: 0, expired: 0,
};
      ...
      <div className="flex items-center justify-between">
        ...
        {canExchange && (
          <button
            ...
            title={limitReached ? "Bạn đang có 5 trao đổi chưa hoàn thành. Hoàn thành bớt trước khi gửi thêm." : undefined}
...
function EmptyState({ tab }: { tab: "active" | "completed" | "rejected" }) {
  ...
  return (
    <div className="text-xs text-muted-foreground py-6 text-center bg-muted/30 rounded-xl">
      {msg}
    </div>
  );
}
...
function TabBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className={`px-3 py-2 text-xs font-semibold border-b-2 -mb-px transition-colors ${active ? "border-primary text-primary" : "border-transparent text-muted-foreground"}`}>
      {children}
    </button>
  );
}
...
function CreateExchangeDialog({
  ...
  const submit = async () => {
    setErr(null);
    if (!selectedReqId) { setErr("Chọn doanh nghiệp của bạn"); return; }
    if (!wantText.trim() || !giveText.trim()) { setErr("Vui lòng điền đầy đủ thông tin"); return; }
    setBusy(true);
      ...
      {[1, 2, 3, 4, 5, 6].map((i) => (
        <span
          ...
          className={`h-1.5 rounded-full transition-all ${
            i <= step ? "bg-primary w-4" : "bg-muted w-2"
          }`}
        />
      ))}
    </div>
...
function ExchangeCard({
  e, bizMap, myBusinessIds, onChanged, onOpen,
}: { e: Exchange; bizMap: Map<string, Business>; myBusinessIds: string[]; onChanged: () => void; onOpen: () => void }) {
  const [busy, setBusy] = useState(false);
  ...
  const receiverConfirmAndDo = () => update({ status: "receiver_done", receiver_completed_at: new Date().toISOString() });
  ...
  if (e.status === "pending" && !iAmRequester) {
    ...
    action = <ActionBtn variant="primary" disabled={busy} onClick={requesterMarkDone}><Check className="w-3.5 h-3.5" /> Tôi đã hoàn thành</ActionBtn>;
  } else if (e.status === "requester_done" && !iAmRequester) {
    action = (
      <div className="flex gap-2">
        <ActionBtn variant="primary" disabled={busy} onClick={receiverConfirmAndDo}><Check className="w-3.5 h-3.5" /> Xác nhận & Tôi đã xong</ActionBtn>
    ...
    action = <ActionBtn variant="primary" disabled={busy} onClick={requesterConfirmCompletion}><Check className="w-3.5 h-3.5" /> Xác nhận hoàn thành</ActionBtn>;
  }
  ...
  return (
    <div className="p-3 rounded-2xl bg-card shadow-sm space-y-2 border hover:border-primary/40 transition-colors">
      ...
      <div className="flex items-center justify-between gap-2">
        <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold inline-flex items-center gap-1 ${sc.chip}`}>
...
function ExchangeDetailDialog({
  exchange, bizMap, onClose, myBusinessIds, onChanged,
}: {
  ...
  const timeline: { at: string | null; label: string }[] = [
    ...
    { at: e.status === "rejected" || e.status === "expired" ? e.updated_at : null, label: e.status === "rejected" ? "❌ Bị từ chối" : "⌛ Đã hết hạn" },
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Chi tiết trao đổi</DialogTitle></DialogHeader>
        <div className="space-y-4">
          ...
          <div className="space-y-1 text-sm">
            <div><b>Loại:</b> {e.request_type}</div>
            <div><b>{reqBiz?.name || "Người gửi"} muốn:</b> {e.request_description}</div>
            <div><b>{reqBiz?.name || "Người gửi"} đổi lại:</b> {e.return_description}</div>
          </div>
          ...
          {ACTIVE.includes(e.status) && (
            <div className="flex flex-col gap-2 pt-2 border-t">
              {e.status === "pending" && !iAmRequester && (
                <>
                  <ActionBtn variant="primary" disabled={busy} onClick={() => update({ status: "accepted" })}><Check className="w-3.5 h-3.5" /> Chấp nhận</ActionBtn>
                  <ActionBtn variant="ghost" disabled={busy} onClick={() => update({ status: "rejected" })}><X className="w-3.5 h-3.5" /> Từ chối</ActionBtn>
                </>
              )}
              {e.status === "accepted" && iAmRequester && (
                <ActionBtn variant="primary" disabled={busy} onClick={() => update({ status: "requester_done", requester_completed_at: new Date().toISOString() })}><Check className="w-3.5 h-3.5" /> Tôi đã hoàn thành</ActionBtn>
              )}
              {e.status === "requester_done" && !iAmRequester && (
                <>
                  <ActionBtn variant="primary" disabled={busy} onClick={() => update({ status: "receiver_done", receiver_completed_at: new Date().toISOString() })}><Check className="w-3.5 h-3.5" /> Xác nhận & Tôi đã xong</ActionBtn>
                  <ActionBtn variant="ghost" disabled={busy} onClick={() => update({ status: "accepted", requester_completed_at: null })}><X className="w-3.5 h-3.5" /> Chưa thực hiện</ActionBtn>
                </>
              )}
              {e.status === "receiver_done" && iAmRequester && (
                <ActionBtn variant="primary" disabled={busy} onClick={() => update({ status: "completed", completed_at: new Date().toISOString() })}><Check className="w-3.5 h-3.5" /> Xác nhận hoàn thành</ActionBtn>
              )}
            </div>
          )}
        </div>
...
function BusinessMini({ biz, role }: { biz?: Business; role: string }) {
  ...
  return (
    <Link to={`/dn/${biz.id}`} className="flex-1 flex flex-col items-center text-center gap-1 hover:text-primary min-w-0">
      ...
      <div className="text-[10px] text-muted-foreground">Lv.{biz.level ?? 1} · {biz.points ?? 0} điểm</div>
    </Link>
  );
}
